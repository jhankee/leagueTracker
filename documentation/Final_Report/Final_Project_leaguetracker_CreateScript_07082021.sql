...
CREATE TABLE IF NOT EXISTS `leaguetracker`.`family` (
  ...
  `PrimaryPhone` VARCHAR(10) NULL DEFAULT NULL,
...
DELIMITER $$
USE `leaguetracker`$$
CREATE DEFINER=`hankee`@`%` PROCEDURE `insert_invoice`(player int,family int)
begin
...
USE `leaguetracker`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`hankee`@`%` SQL SECURITY DEFINER VIEW `leaguetracker`.`v_all_rosters` AS select `leaguetracker`.`division`.`divisionName` AS `Division Name`,`leaguetracker`.`team`.`teamName` AS `Team Name`,concat(`leaguetracker`.`player`.`playerLastName`,', ',`leaguetracker`.`player`.`playerFirstName`) AS `Player Name` from ((`leaguetracker`.`player` join `leaguetracker`.`team` on((`leaguetracker`.`player`.`teamID` = `leaguetracker`.`team`.`teamID`))) join `leaguetracker`.`division` on((`leaguetracker`.`team`.`divisionID` = `leaguetracker`.`division`.`divisionID`))) order by `leaguetracker`.`division`.`divisionName`,`leaguetracker`.`team`.`teamName`;
...
CREATE
DEFINER=`hankee`@`%`
TRIGGER `leaguetracker`.`check_invoice_before_delete`
BEFORE DELETE ON `leaguetracker`.`family`
FOR EACH ROW
BEGIN
